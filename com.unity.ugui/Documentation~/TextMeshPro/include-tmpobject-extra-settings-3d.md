...
|**Is Scale Static**||Enable to set the scale of this TextMesh Pro GameObject to static, which excludes the object from the InternalUpdate callback used to handle SDF Scale updates when the object or its parent(s) are scaled.|
|**Orthographic Mode**||Enable this option when creating camera-aligned text with an orthographic camera. <br/><br/> It prevents the TextMesh Pro shader from using perspective correction.|
...
|**Emoji Fallback Support**||Disable this option to turn off [color emoji](ColorEmojis.md) support for the TextMesh Pro GameObject.|
| **Sprite Asset** |||
| **Style Sheet Asset**||Style sheet used by the text object, overriding the default style sheet.|
|**Font Features**||Control which [OpenType FontFeatures](TMPObjects.md#opentype-fontfeature) are enabled.|
